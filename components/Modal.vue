<template>
    <transition name="slide-fade-fast">
        <div v-if="modalVisible"
            ...
            @click.self="close">
            <div class="bg-white w-full max-w-md  rounded-lg overflow-hidden shadow-lg dark:bg-gray-700">
                ...
                <!-- Action buttons -->
                <div class="flex items-center justify-end gap-2 px-4 py-2 border-t dark:border-gray-600">
                    <slot name="footer"></slot>
                    <!-- Default action -->
                    <Button v-if="showCloseButton" color="error" variant="bordered" @click="close">Close</Button>
                </div>
            </div>
        </div>
    </transition>
